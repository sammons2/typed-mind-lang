...
UserDTO %
  - id: string
    ...
    const typeDeviation = result.deviations.find(d => 
      d.entityName === 'UserService' && 
      d.property === 'type'
    );
...
UserDTO %
  - id: string
    ...
    const methodDeviation = result.deviations.find(d => 
      d.entityName === 'UserService' && 
      d.property === 'methods.missing'
    );
...
UserDTO %
  ...
  - updatedAt?: Date
    ...
    const missingFieldDeviation = result.deviations.find(d => 
      d.entityName === 'UserDTO' && 
      d.property === 'field.createdAt'
    );
...
createUser :: createUser(data: UserCreateData) => User
  ...
  -> User
    ...
    const signatureDeviation = result.deviations.find(d => 
      d.entityName === 'createUser' && 
      d.property === 'signature'
    );
  ...
  it('should handle invalid TMD syntax', () => {
    ...
    const syntaxDeviation = result.deviations.find(d => 
      d.entityName === '<parsing>' && 
      d.property === 'syntax'
    );
...
UserDTO %
  ...
  - email: string
    ...
    const versionWarning = warnings.find(d => 
      d.entityName === 'IndexApp' && 
      d.property === 'version'
    );
