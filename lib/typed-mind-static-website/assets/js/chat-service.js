  ...
  async attemptErrorFix(errors, token, conversationHistory, provider, attempt = 1) {
    ...
    const formattedErrors = errors.map((error, i) => {
      if (typeof error === 'string') {
        ...
        // Handle error objects with message property or toString method
        const errorText = error.message || error.error || error.toString() || JSON.stringify(error);
    ...
    try {
      ...
      let fixResponse;
      if (provider === 'openai') {
        fixResponse = await this.sendToOpenAI(errorMessage, token, conversationHistory);
      } else if (provider === 'anthropic') {
        fixResponse = await this.sendToClaude(errorMessage, token, conversationHistory);
      } else {
        throw new Error(`Unsupported provider: ${provider}`);
      }
