...
IMPORTANT: When a user asks you to "write", "create", "generate", or "make" a program or code:
1. First use getCode to check current editor content
2. If editor is empty, use appendCode to add the entire program
3. If editor has content, use editCode or replaceCode to modify it
4. Always apply generated code directly to the editor using tools

Do NOT just show code in the chat - actively use the tools to place it in the editor.
...
Always respond with clear, well-structured TypedMind code when generating architecture definitions.`;
  ...
  async attemptErrorFix(errors, token, conversationHistory, provider, attempt = 1) {
    ...
    try {
      const fixResponse = await this.sendMessage(errorMessage, provider, conversationHistory);
      ...
      if (fixResponse.validationResult && !fixResponse.validationResult.valid) {
        return await this.attemptErrorFix(
          ...
          token,
          [...conversationHistory, { role: 'user', content: errorMessage }, fixResponse],
          provider,
          attempt + 1
        );
      }
