...
export class GrammarDocGenerator {
  generateMarkdown(): string {
    ...
    sections.push('');
    sections.push('# File with imports and exports');
    sections.push('UserService @ src/services/user.ts:');
    sections.push('  <- [Database, UserModel]');
    sections.push('  -> [createUser, getUser, updateUser]');
    sections.push('  "Handles user business logic"');
    sections.push('');
    sections.push('# Function with full specification');
    sections.push('createUser :: (data: UserCreateDTO) => Promise<User>');
    sections.push('  "Creates a new user in the database"');
    sections.push('  <- UserCreateDTO');
    sections.push('  -> User');
    sections.push('  ~> [validateUser, Database.insert, sendWelcomeEmail]');
    sections.push('  ~ [UserList, UserCount]');
    sections.push('  $< [DATABASE_URL, SMTP_HOST]');
    sections.push('');
    sections.push('# DTO with fields');
    sections.push('UserCreateDTO % "Data for creating a new user"');
    sections.push('  - name: string "User full name"');
    sections.push('  - email: string "User email address"');
    sections.push('  - password: string "User password (will be hashed)"');
    sections.push('  - age?: number "User age (optional)"');
    sections.push('');
    sections.push('# UI Components');
    sections.push('App &! "Root application component"');
    sections.push('  > [Header, MainContent, Footer]');
    sections.push('');
    sections.push('UserList & "Displays list of users"');
    sections.push('  < [MainContent]');
    sections.push('  > [UserCard]');
    sections.push('');
    sections.push('# Runtime parameters');
    sections.push('DATABASE_URL $env "PostgreSQL connection string" (required)');
    sections.push('SMTP_HOST $env "Email server host"');
    sections.push('  = "localhost"');
    sections.push('');
    sections.push('# Dependencies');
    sections.push('express ^ "Web framework" v4.18.0');
    sections.push('@types/node ^ "Node.js type definitions" v20.0.0');
    sections.push('```');
